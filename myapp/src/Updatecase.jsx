...
function UpdateCase() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!fileInputRef.current.files[0]) {
        // Show error if file isn't uploaded
    ...
    // Validate the form before uploading the file
    if (!validateForm()) {
        return;
    }
    ...
    try {
        ...
        if (uploadRes.data.message === "Success") {
            // Update formData with the file's response data
            const updatedFormData = { 
                ...formData, 
                witness: uploadRes.data.filename 
            };

            // Update file_id in formData
            updatedFormData.witness = uploadRes.data.file_id;

            console.log("Updated Form Data:", updatedFormData);

            // Send the case update request with the updated form data
            const caseRes = await axios.put(`http://localhost:5000/api/case/update/${id}`, updatedFormData);
